...
export default template({
  ...
  created: function() {
    ...
    this.statement.bulk_update = false;
    if (
      ["verifiable", "reconcilable", "manually_actionable"].indexOf(
        this.statement.previousType
      ) !== -1
    ) {
  ...
  methods: {
    ...
    updatePositionHeld: function() {
      ...
      if (this.statement.statement_uuid) {
        ...
        updateData.statement = this.statement.statement_uuid.replace(
          /^(Q\d+)[^\d]/,
          "$1$"
        );
      }
      ...
      references[
        wikidataClient.getReferencePropertyID(this.page.reference_url)
      ] = {
        value: this.page.reference_url,
        type: "string"
      };

      references[wikidataClient.getPropertyID("reference retrieved")] = {
        value: this.statement.verified_on,
        type: "time"
      };
      ...
      if (
        !this.page.executive_position &&
        this.statement.parliamentary_group_item
      ) {
        qualifiers[
          wikidataClient.getPropertyID("parliamentary group")
        ] = this.statement.parliamentary_group_item;
      }
      if (
        !this.page.executive_position &&
        ...
        qualifiers[
          wikidataClient.getPropertyID("electoral district")
        ] = this.statement.electoral_district_item;
      }
      if (this.statement.parliamentary_term_item) {
        qualifiers[
          wikidataClient.getPropertyID("parliamentary term")
