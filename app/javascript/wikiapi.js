...
function getReferenceForURLFromAPIClaims(
  references,
  referenceURLProp,
  referenceURL
) {
  ...
  return references.find(function(r) {
    var snak,
      snaks = r.snaks[referenceURLProp];
    if (!snaks) {
  ...
  if (qualifiersFromAPI) {
    for (i = 0; i < qualifiersToCheck.length; ++i) {
      ...
      if (existingQualifiersForProperty[0].snaktype != "value") {
        throw new Error(
          "Unexpected snaktype " +
            existingQualifiersForProperty[0].snaktype +
            " found on the " +
            qualifiersToCheck[i] +
            " qualifier"
        );
      ...
      if (
        existingQualifiersForProperty[0].datavalue.type != "wikibase-entityid"
      ) {
        throw new Error(
          "Unexpected datavalue type " +
            existingQualifiersForProperty[0].datavalue.type +
            " found on the " +
            qualifiersToCheck[i] +
            " qualifier"
        );
      ...
      if (existingQualifiersForProperty[0].datavalue.value.id == newValue) {
        ...
        throw new Error(
          "The existing item for the " +
            qualifiersToCheck[i] +
            " qualifier was " +
            existingQualifiersForProperty[0].datavalue.value.id +
...
var wikidataItem = function(spec) {
  ...
  my.ajaxSetQualifier = function(qualifierDetails) {
    wikidata.log(
      "Setting " + wikidata.getPropertyName(qualifierDetails.qualifierProperty)
    );
    return wikidata
      .ajaxAPI(true, "wbsetqualifier", {
  ...
  my.makeSetQualifierThenFunction = function(statementToCreate) {
    return function(data) {
      console.log(
        "in 'then' function for " + statementToCreate + " data was: ",
        data
      );
      checkForError(data);
  ...
  my.createQualifiers = function(newClaim, newQualifiers) {
    var requestChain,
      i,
      statementsToCreate = Object.keys(newQualifiers).map(function(
        qualifierProperty
      ) {
        return {
    ...
    console.log(
      "There are " + statementsToCreate.length + " statements to create...."
    );
  ...
  my.getReferencePropertyInUse = function(newReferences) {
    var found,
      ...
      knownReferencePropertyLabels = ["Wikimedia import URL", "reference URL"],
      knownReferencePropertyIDs = knownReferencePropertyLabels.map(l =>
        wikidata.getPropertyID(l)
      );
    found = newReferenceProperties.find(k =>
      knownReferencePropertyIDs.includes(k)
    );
    if (!found) {
      throw new Error(
        "Couldn't find a reference property ID in " + newReferenceProperties
      );
    }
    return found;
  };
  ...
  that.updateOrCreateClaim = function(baseRevisionID, claimData) {
    ...
    if (claimData.statement) {
      ...
      return my.createBareClaimDeferred(claimData).then(function(statement) {
        ...
        return my.updateClaim(
          Object.assign({}, claimData, { statement: statement })
        );
      });
    }
  };
...
var wikidata = function() {
  ...
  that.ajaxAPIBasic = function(data) {
    ...
    return Axios.post(that.apiURL, params, { responseType: "json" }).then(
      function(response) {
  ...
  that.getPropertyName = function(property) {
    return Object.keys(that.getProperties()).find(
      key => that.getProperties()[key] === property
    );
  };
  ...
  that.search = function(name, wikipediaToSearch, language) {
    ...
    var searchWikipedia = new Promise(function(resolve) {
      jsonpPromise(
        ...
        var titles = allResults.fromWikipedia.map(function(result) {
          return result.title;
        });
    ...
    var addWikidataItemsToWikipediaResults = function(sitelinks) {
      ...
      for (let [wikidataItem, sitelinkData] of Object.entries(
        sitelinks.entities
      )) {
    ...
    return Promise.all([searchWikidata, searchWikipedia]).then(function() {
      return allResults;
    });
  };
